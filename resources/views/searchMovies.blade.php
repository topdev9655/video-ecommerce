...
@section('title', 'movies search result')
...
<script>
    ...
    const getFilters = (e) => {
       ...
       document.location.href = "<?= url('movies/search/') ?>/{{ $keyword }}/" + JSON.stringify(d);
    } 
...
<div class="d-sm-flex align-items-center justify-content-between mt-4 mb-3">
    <h1 class="h5 mb-0 text-gray-900">Search result for movie: "{{ $keyword }}"</h1>
    <a href="{{ $keyword ? url('movies/search/'.$keyword) : route('movies') }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">Reset Filters <i class="fas fa-times fa-sm text-white-50 ml-2"></i></a>
 </div>
 <!-- Content Row -->
 <div class="row">
